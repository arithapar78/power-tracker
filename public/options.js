...
class OptionsManager {
  ...
  setupEventListeners() {
    try {
      ...
      this.changeEventListener = (e) => {
        if (e.target.id === 'trackingEnabled' || e.target.id === 'samplingInterval') {
          this.updateSetting(e);
        }
        else if (e.target.id === 'enablePopups') {
          this.updateNotificationSetting('enabled', e.target.checked);
        }
        else if (e.target.id === 'notificationInterval') {
          this.updateNotificationSetting('notificationInterval', parseInt(e.target.value));
        }
        else if (e.target.id === 'notificationDuration') {
          this.updateNotificationSetting('duration', parseInt(e.target.value));
        }
  ...
  async loadNotificationSettings() {
    try {
      ...
      if (response && response.success && response.settings) {
        ...
        // Set enable popups checkbox
        this.safeSetCheckbox('enablePopups', Boolean(settings.enabled));
        // Set notification interval dropdown
        this.safeSetValue('notificationInterval', settings.notificationInterval || 300000);
        // Set notification duration dropdown
        this.safeSetValue('notificationDuration', settings.duration || 5500);
      }
    } catch (error) {
      console.error('Failed to load notification settings:', error);
      // Set defaults
      this.safeSetCheckbox('enablePopups', true);
      this.safeSetValue('notificationInterval', 300000);
      this.safeSetValue('notificationDuration', 5500);
    }
  }
  ...
  async updateNotificationSetting(key, value) {
    try {
      ...
      this.showError('Failed to update notification settings: ' + error.message);
    }
  }
  ...
  generateHistoryFromStats(stats) {
    // Return empty history if no actual data - don't generate fake data
    return [];
  }
  
  extractCategoriesFromStats(stats) {
    // Return empty categories if no data - don't generate fake data
    return {
  ...
  generateModelPerformance(stats) {
    // Return empty model performance if no data - don't generate fake data
    return {
      'GPT-4': { efficiency: 0, avgReduction: 0 },
      'GPT-3.5': { efficiency: 0, avgReduction: 0 },
      'Claude': { efficiency: 0, avgReduction: 0 },
      'Gemini': { efficiency: 0, avgReduction: 0 },
      'Grok-4': { efficiency: 0, avgReduction: 0 },
      'DeepSeek R1': { efficiency: 0, avgReduction: 0 }
    };
  }
  
  getFallbackPromptAnalytics() {
    // Return empty data when no actual optimization data exists
    return {
      totalPromptsOptimized: 0,
      totalTokensSaved: 0,
      averageReduction: 0,
      topOptimizationLevel: 'Balanced',
      tokenReductionHistory: [],
      modelPerformance: {
        'GPT-4': { efficiency: 0, avgReduction: 0 },
        'GPT-3.5': { efficiency: 0, avgReduction: 0 },
        'Claude': { efficiency: 0, avgReduction: 0 },
  ...
  updatePromptGeneratorAnalytics(data) {
    ...
    // Calculate energy saved based on tokens saved
    // Using average of ~0.03 kWh per 1000 tokens (30 mWh per 1000 tokens)
    // Formula: (totalTokensSaved / 1000) * 30 = mWh saved
    const totalTokensSaved = data.totalTokensSaved || 0;
    const energySavedMwh = (totalTokensSaved / 1000) * 30;
    this.safeSetTextContent('energySaved', energySavedMwh.toFixed(1));

    // Update model-specific average reductions
    if (data.modelPerformance) {
      this.safeSetTextContent('gpt4AvgReduction', (data.modelPerformance['GPT-4']?.avgReduction || 0).toFixed(1) + '%');
      this.safeSetTextContent('claudeAvgReduction', (data.modelPerformance['Claude']?.avgReduction || 0).toFixed(1) + '%');
      this.safeSetTextContent('grokAvgReduction', (data.modelPerformance['Grok-4']?.avgReduction || 0).toFixed(1) + '%');
      this.safeSetTextContent('deepseekAvgReduction', (data.modelPerformance['DeepSeek R1']?.avgReduction || 0).toFixed(1) + '%');
    }

    // Update optimization category counts
    if (data.optimizationCategories) {
      this.safeSetTextContent('politenessCount', (data.optimizationCategories['Politeness Markers'] || 0).toLocaleString());
      this.safeSetTextContent('intensifiersCount', (data.optimizationCategories['Intensifiers'] || 0).toLocaleString());
      this.safeSetTextContent('verboseCount', (data.optimizationCategories['Verbose Phrases'] || 0).toLocaleString());
      this.safeSetTextContent('fillerCount', (data.optimizationCategories['Filler Words'] || 0).toLocaleString());
    }
  }
...
function showPromptGeneratorInstructions() {
  ...
  modal.innerHTML = `
    ...
    <h2 style="color: #1f2937; margin-bottom: 16px; font-size: 24px;">Access the Prompt Generator</h2>
    <p style="color: #4b5563; margin-bottom: 24px; line-height: 1.6;">
      To use the AI Prompt Generator and optimize your prompts for energy efficiency:
    </p>
    <ol style="text-align: left; color: #4b5563; line-height: 1.8; margin-bottom: 24px; padding-left: 20px;">
      <li>Click the <strong>Power Tracker extension icon</strong> in your browser toolbar</li>
      <li>The popup will open with the Prompt Generator tab</li>
      <li>Enter your prompt and click "Generate Optimized Prompt"</li>
      <li>Save 15-45% energy per AI query!</li>
    </ol>
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if we should show prompt generator instructions
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('openPromptGenerator') === 'true') {
      showPromptGeneratorInstructions();
    }
  });
} else {
  optionsManager = new OptionsManager();
